...
We have three scenario implementations (curl, java-manual, java-generated) and three core scenarios (S1–S3). The previous CI ran a single undifferentiated `mvn verify` job that didn't test curl at all, couldn't isolate individual scenario failures, and gave no visibility into which client/scenario combinations passed or failed.
