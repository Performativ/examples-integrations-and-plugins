...
This is the intended feedback loop: generated tests fail → spec issue is identified → the API is corrected or the spec is regenerated from the API gateway → updated `openapi.json` is committed here → generated tests pass.

## Consequences

- `java-generated` failures are **not bugs in the test** — they indicate spec drift that needs fixing upstream (either correcting the API or regenerating the spec).
- The `java-manual` implementation of the same scenario proves the API itself works; the `java-generated` failure pinpoints the spec gap.
