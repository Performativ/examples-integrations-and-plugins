...
1. Enable mTLS access for your plugin in the Performativ UI
2. Download the **PEM bundle** containing a client certificate and private key (one-time download)
...
Enable mTLS access for your plugin through the Performativ admin panel:
...
When a certificate is approaching expiry, rotate it through the Performativ UI:
...
Certificates have a defined expiry date (`certificate_expires_at`). Monitor this and rotate before expiry. The certificate status is visible in the plugin details in the admin panel.
